...
also a reference to native.genrule that inspired this rule's design (though there are significant
differences).
...
See the _ATTRS variable in this file for attribute documentation.
...
The "process_assets" rule above will execute the Bash command in "cmd" to process "rust.png",
"teapot.3ds", "//assets:wood.jpg", and every file in the "//assets:models" rule, producing a
corresponding .tags and .md5 file for each of them, under the following paths:
...
You can create downstream rules, for example native.filegroup or native.genrule (or another maprule)
that put this rule in their "srcs" and get all the .tags and .md5 files.

## Tips and tricks

-   Unless the command is trivial, don't try to write it in "cmd". Due to the inevitably necessary
    quoting and escaping in the BUILD file, it's a lot easier and maintainable to create a script
    file (e.g. "foo.sh" in bash_maprule or "foo.bat" in cmd_maprule) for the commands, add it to
    "tools" and "add_env", and replace the "cmd" with just "$MAPRULE_FOO". Doing so also avoids
    hitting command line length limits.
-   The rule defines several environment variables that the command may reference. All of these
    envvars start with "MAPRULE_". You can add your own envvars with the "add_env" attribute.
-   Values in the "add_env" attribute may use "$(location)" references. They may also use the same
    placeholders as the "outs_templates" attribute. For example you can use this mechanism to pass
    extra "$(location)" references of "tools" or "srcs" to the actions.

## FAQ

-   Q: What's the motivation for maprule? What's wrong with native.genrule?
-   A: native.genrule creates a single action for all of its inputs. It requires specifying all
       output files. Finally, it can only create Bash commands.
       Maprule supports parallel processing of its inputs and doesn't require specifying all
       outputs, just templates for the outputs. And not only Bash is supported (via bash_maprule)
       but so are cmd.exe-style commands via cmd_maprule.

-   Q: native.genrule.cmd supports $(location) expressions, how come maprule.cmd doesn't?
-   A: Maprule deliberately does not support this feature to avoid pitfalls with quoting and
       escaping, and potential issues with paths containing spaces. Instead, maprule exports
       environment variables for the input and outputs of the action, and allows the user to define
       extra envvars. These extra envvars do support $(location) expressions, so you can pass paths
       of labels in "srcs" and "tools".

-   Q: Why are all envvars exported with the "MAPRULE_" prefix?
-   A: To avoid conflicts with other envvars, whose names could also be attractive outs_templates
       names.

-   Q: Why do "outs_templates" and "add_env" keys have to be uppercase?
-   A: Because they are exported as all-uppercase envvars, so requiring that they be declared as
       uppercase gives a visual cue of that. It also avoids clashes resulting from mixed
       lower-upper-case names like "foo" and "Foo".

-   Q: Why don't "outs_templates" and "add_env" keys have to start with "MAPRULE_" even though they
       are exported as such?
-   A: For convenience. It seems to bring no benefit to have the user always type "MAPRULE_" in
       front of the name when the rule itself could as well add it.

-   Q: Why are all outputs relative to `<maprule_pkg>/<maprule_name>_out/` ?
-   A: To ensure that maprules in the same package and with the same outs_templates produce distinct
       output files.

-   Q: Why aren't "outs_templates" keys available as placeholders in the values of "add_env"?
-   A: Because "add_env" is meant to be used for passing extra $(location) information to the
...
_ATTRS = {
    ...
    "tools": attr.label_list(
        ...
        doc = "Tools used by the command. The \"cmd\" attribute, and the values of the " +
              ...
              "similar to the native.genrule() rule.",
    ),
    "outs_templates": attr.string_dict(
        allow_empty = False,
        mandatory = True,
        doc = "Templates for the output files. Each key defines a name for an output file " +
              ...
              "templates:<br/><br/>" +
              "- \"{src}\": same as \"{src_dir}/{src_name}\"<br/>" +
              "- \"{src_dir}\": package path of the source file, and a trailing \"/\"<br/>" +
              "- \"{src_name}\": base name of the source file<br/>" +
              "- \"{src_name_noext}\": same as \"{src_name}\" without the file extension<br/>" +
              "You may also add extra path components to the templates, as long as the path " +
              ...
              "\"<bazel_bin>/path/to/maprule/<maprule_name>\" + \"_outs/\".",
    ),
    "cmd": attr.string(
        mandatory = True,
        doc = "The command to execute. May contain $(location) expressions for labels " +
              "declared in the \"srcs\" and \"tools\" attributes, these will be replaced by " +
              "the path of the referenced label. The following environment variables will be " +
              "available to this command:<br/><br/>" +
              "- MAPRULE_SRC: the path of the current file from \"foreach_srcs\"<br/>" +
              "- MAPRULE_SRCS: the space-separated paths of all files in the \"srcs\" " +
              "  attribute<br/>" +
              "- MAPRULE_<OUT>: for each key name <OUT> in the \"outs_templates\" attribute, " +
              "  this is the path of the respective output file for the current source<br/>" +
